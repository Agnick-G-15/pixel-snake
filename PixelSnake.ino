...
void graphic_generate_characters() {
    ...
    */
    ...
    delay(1); // Wait for CGRAM to be written
}
...
void game_calculate_logic() {
    ...
    if (snakePosHistory[0].x < 0 || snakePosHistory[0].x >= GRAPHIC_WIDTH ||
        snakePosHistory[0].y < 0 || snakePosHistory[0].y >= GRAPHIC_HEIGHT) {
        gameState = GAME_LOSE;
        return;
    }
...
void loop() {
    ...
    if (digitalRead(BUTTON_RIGHT) == HIGH) {
        if (debounce_activate_edge(&debounceCounterButtonRight) && !thisFrameControlUpdated) {
            switch (gameState) {
                case GAME_PLAY:  // Normal turn right
                    switch (snakeDirection) {
                ...
                case GAME_MENU: // Start game
                case GAME_LOSE: // Restart after losing
                case GAME_WIN:  // Restart after winning
                    game_init();
                    break;
            }
        }
